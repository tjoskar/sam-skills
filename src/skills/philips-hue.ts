...
export class PhilipsHue implements Skill {
  ...
  private async getBridgeIp() {
    const result = await fetch('https://discovery.meethue.com/').then((r) =>
      r.json()
    );
  ...
  private async getLights(ip: string): Promise<Lamps> {
    return fetch(`http://${ip}/api/${this.#username}/lights`).then((r) =>
      r.json()
    );
  }
  ...
  private async changeState(ip: string, id: string, on: boolean) {
    ...
    return fetch(`http://${ip}/api/${this.#username}/lights/${id}/state`, {
      method: 'PUT',
  ...
  private findLampIds(name: string, lamps: Lamps): string[] {
    ...
    return Object.entries(lamps)
      .filter(([id, lamp]) => lamp.name.toLowerCase() === name.toLowerCase())
      .map(([id]) => id);
  }
